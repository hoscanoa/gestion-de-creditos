...
public class SimpleClienteManager implements ClienteManager {
	...
	public boolean agregarCliente(String cli_ruc, String cli_razonSocial,
	...
	public void aumentarLineaCredito(Cliente cliente, double incremento) {
		cliente.setCli_lineaCreditoTotal(cliente.getCli_lineaCreditoTotal()
				+ (float) incremento);
		cliente.setCli_lineaCreditoSaldo(cliente.getCli_lineaCreditoSaldo()
				+ (float) incremento);
	...
	public boolean comprobarLineaCredito(Pedido pedido) {
		...
		try {
			...
			if (cliente.getCli_lineaCreditoSaldo() >= (float) pedido
					.getMontoTotal()) {
				return true;
			} else {
				return false;
